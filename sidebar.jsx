...
function Sidebar({ view, go, progress, onPrint, locked, footer }){
  ...
  return (
    <aside className="no-print app-sidebar" style={{
      ...
      <div style={{ padding:"14px 18px 18px", borderTop:"1px solid var(--line)" }}>
        ...
        <a href="mailto:hello@bloomcultureflowers.com?subject=Help%20with%20my%20DIY%20Flower%20Planner" 
          style={{ display:"flex", alignItems:"center", gap:7, marginTop:4, padding:"6px 8px", borderRadius:8,
            textDecoration:"none", color:"var(--ink-faint)", fontSize:11.5, fontWeight:600, transition:"background .15s" }}
          onMouseEnter={e=>e.currentTarget.style.background="var(--cream)"}
          onMouseLeave={e=>e.currentTarget.style.background="transparent"}>
          <Icon name="lifebuoy" size={14} style={{ color:"var(--sage-deep)", flexShrink:0 }}/>
          Need help? Email us
        </a>
        <div style={{ fontSize:10, color:"var(--ink-faint)", padding:"8px 8px 0", lineHeight:1.4 }}>
          © {new Date().getFullYear()} Bloom Culture™. All rights reserved.
        </div>
      </div>
      )}
    </aside>
...
function NavItem({ active, onClick, icon, label, sub, num, ring, clay, locked }){
  ...
  return (
    <button onClick={onClick} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{
        ...
        transition:"background .16s", position:"relative", color:"var(--ink)", opacity: locked ? .58 : 1
      }}>
      ...
      {locked
        ? <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="var(--ink-faint)" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="5" y="11" width="14" height="9" rx="2"/><path d="M8 11V7a4 4 0 0 1 8 0v4"/></svg>
